...
class WorldMap extends Component {
      ...
      .then(response => {
        ...
        response.json().then(lspdata => {
          this.setState({
            ...
            destinations: [...lspdata[0].paths.map((path) => ({ 
  ...
  render() {
    ...
    return (
      <svg width={ '100vw' } height={ '100vh' } viewBox="0 0 1200 950">
        ...
        <g className="markers">
          {
            this.state.destinations.map((city, i) => (
              <line
                key={ `marker-${i}` }
                x1={this.projection()(this.state.cities[0].coordinates)[0]}
                y1={this.projection()(this.state.cities[0].coordinates)[1]}
                x2={ this.projection()(city.coordinates)[0] }
                y2={ this.projection()(city.coordinates)[1] }
                fill="blue"
                stroke="green"
                strokeWidth={5}
                className="marker"
                onClick={ () => this.handleMarkerClick(i) }
              ><title>path: { city.path }</title>
              </line>
            ))
          }
        </g>
